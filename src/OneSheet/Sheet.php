...
class Sheet
{
    ...
    public function __construct($freezePaneCellId = null)
    {
        ...
        if (1 == preg_match('~[A-Z]+([1-9]?[0-9]+)$~', $freezePaneCellId, $match)) {
            $this->spl->fwrite('<sheetViews><sheetView tabSelected="1" workbookViewId="0" showGridLines="true" showRowColHeaders="1"><pane ySplit="'. array_pop($match) .'" topLeftCell="'. $freezePaneCellId .'" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews>');
        }
